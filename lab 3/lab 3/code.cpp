...
struct struck_ochered* get_ochered(void) // создание элемента структуры
{
...
void ochered_stek(void) { // создание связи между элементами структуры
	...
	if (head_ochered == NULL) {
		...
		q->next = head_ochered; // добавляем элемент в начало структуры
		head_ochered = q;
	}
}

void review_ochered(int razmer) // выводим данные из очереди на экран
{
...
/*void reviwe_one_day() { // функция относится к 1 заданию
